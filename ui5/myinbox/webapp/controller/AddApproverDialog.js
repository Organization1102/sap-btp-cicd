sap.ui.define([
	...
	return ManagedObject.extend("kupit.flexiblewf.myinbox.controller.AddApproverDialog", {
		constructor: function(oView) {
			this._oView = oView;
			this._oControl = sap.ui.xmlfragment(oView.getId(), "kupit.flexiblewf.myinbox.view.AddApproverDialog", this);
			this._bInit = false;
		},
		...
		_onInputValueHelpRequest: function(oEvent) {
			...
			var fnConfirm = async function(oConfirmEvent) {
				...
				if (oSelectDialog.getControl().getBinding("items")) {
					this.getView().byId("AddApproverDialog").setBusy(true);
					var oModel = this.getOwnerComponent().getModel();
					...
					if ( selectedAppr.mail != undefined && selectedAppr.mail != null && selectedAppr.mail[0] != undefined && selectedAppr.mail[0]!= null ){
						...
						if (resWD.data.length > 1){
							...
							this.getView().byId("AddApproverDialog").setBusy(false);
		...
		_onOkPress: function() {
			...
			newApprRow.approverName = selectedAppr.userPrincipalName;
			newApprRow.familyName = selectedAppr.surname;
			newApprRow.givenName = selectedAppr.givenName;
			...
			var oApprListTable = this.getView().byId("createWFApprovers"+myAppr.level+"Table");
		...
		onInit: function() {
			this.getView().byId("AddApproverDialog").setBusyIndicatorDelay(10);
